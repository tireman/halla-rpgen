...
void NpolSBSBeamline::ConstructGEpLead(){
  ...
  G4Box *lead_wall2 = new G4Box("lead_wall2", 5.0*cm/2.0, 1.00*m/2/*24.0*inch/2.0*/, 3.5*m/2 /*0.5*(zstop_lead_wall2 - zstart_lead_wall2)*/ );
...
void NpolSBSBeamline::Place(G4LogicalVolume *motherLV) {
  ...
  if( ExpType == "kGEp" || ExpType == "kGMn"){
	...
	PlaceRectangular(lead_wall2_log,motherLV,"lead_wall2_phys", -10*cm + xpos_lead_wall2, 0, 5.1*m /*zpos_lead_wall2*/, 0.0*deg,-3.0*deg,0.0*deg);
