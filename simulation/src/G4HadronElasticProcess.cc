...
// G4cout<<part->GetParticleName()<<"; "<<kineticEnergy/CLHEP::GeV<<" GeV; r = "<<ratio<<G4endl;
  ...
  if( (gPolRot) && (iz==1) && (gPolRot->polFlag)){
    ...
    if(nsec){
      terpart = result->GetSecondary(0)->GetParticle();
    }
    primpart = (G4DynamicParticle*)track.GetDynamicParticle();
    secpart = *primpart;
    G4ThreeVector odir = primpart->GetMomentumDirection();
    odir = odir + outdir -G4ThreeVector(0.0,0.0,1.0);
    secpart.SetKineticEnergy(ekin);
    secpart.SetMomentumDirection(odir.unit());
